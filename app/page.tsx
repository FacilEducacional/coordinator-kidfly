...
function MuralScreen({ avisos, user, usuarios, dest, perms, onAC, onAU, onAD }: any) {
  ...
  async function criar() { if (!tit.trim()) return; var d: any = { escola_id: user.escola_id, autor_id: user.id, titulo: tit, urgente: urg }; if (de) d.data_evento = de; var r = await supabase.from("avisos").insert(d).select().single(); if (r.data) { if (atr === "todos") await supabase.from("aviso_destinatarios").insert({ aviso_id: r.data.id, todos: true }); else { var ds = su.map(function(uid) { return { aviso_id: r.data.id, usuario_id: uid, todos: false }; }); if (ds.length > 0) await supabase.from("aviso_destinatarios").insert(ds); } setLista([r.data, ...lista]); setTit(""); setDe(""); setUrg(false); setAtr("todos"); setSu([]); setSf(false); setMsg("Publicado!"); if (onAC) onAC(r.data); } }
  function startEd(a: any) { setEd(a); setEt(a.titulo); setEdt(a.data_evento || ""); setEu(a.urgente); var di = dest.filter(function(d: any) { return d.aviso_id === a.id; }); if (di.some(function(d: any) { return d.todos; })) { setEa("todos"); setEsu([]); } else { setEa("selecionar"); setEsu(di.filter(function(d: any) { return d.usuario_id; }).map(function(d: any) { return d.usuario_id; })); } }
  async function salvar() { if (!ed) return; var ud: any = { titulo: et, urgente: eu, data_evento: edt || null }; await supabase.from("avisos").update(ud).eq("id", ed.id); await supabase.from("aviso_destinatarios").delete().eq("aviso_id", ed.id); if (ea === "todos") await supabase.from("aviso_destinatarios").insert({ aviso_id: ed.id, todos: true }); else { var ds = esu.map(function(uid) { return { aviso_id: ed.id, usuario_id: uid, todos: false }; }); if (ds.length > 0) await supabase.from("aviso_destinatarios").insert(ds); } setLista(lista.map(function(a: any) { return a.id === ed.id ? Object.assign({}, a, ud) : a; })); setEd(null); setMsg("Atualizado!"); if (onAU) onAU(); }
