...
function GestaoScreen({ user, turmas, usuarios, perms, onUpdate }: any) {
  ...
  async function addProf() {
    ...
    var limRes = await supabase.from("escolas").select("limite_usuarios").eq("id", user.escola_id).single();
    var limite = limRes.data?.limite_usuarios || 999;
    if (usuarios.length >= limite) { setMsg("Limite de " + limite + " usuarios atingido! Contate o suporte KidFly."); return; }
    setInviting(true);
...
function SuperAdminPanel({ user, onLogout }: any) {
  var _a = useState("sa-dash"); var tab = _a[0]; var setTab = _a[1];
  ...
  var _f = useState(false); var showNew = _f[0]; var setShowNew = _f[1];
  var _g = useState(""); var novaEscola = _g[0]; var setNovaEscola = _g[1];
  var _h = useState(10); var novoLimite = _h[0]; var setNovoLimite = _h[1];
  var _i = useState(""); var novaCoordEmail = _i[0]; var setNovaCoordEmail = _i[1];
  var _j = useState(""); var novaCoordNome = _j[0]; var setNovaCoordNome = _j[1];
  var _k = useState(false); var creating = _k[0]; var setCreating = _k[1];
  var _l = useState<any>(null); var editEscola = _l[0]; var setEditEscola = _l[1];
  var _m = useState(""); var editNome = _m[0]; var setEditNome = _m[1];
  var _n = useState(10); var editLimite = _n[0]; var setEditLimite = _n[1];

  async function loadAll() {
    setLd(true);
    var r = await Promise.all([
      supabase.from("escolas").select("*").order("created_at", { ascending: false }),
      supabase.from("usuarios").select("*, escolas(nome)").neq("papel", "superadmin").order("nome", { ascending: true })
    ]);
    setEscolas(r[0].data || []); setAllUsers(r[1].data || []);
    setLd(false);
  }
  ...
  async function criarEscola() {
    ...
    setCreating(true);
    var er = await supabase.from("escolas").insert({ nome: novaEscola, ativa: true, limite_usuarios: novoLimite }).select().single();
    if (er.data && novaCoordEmail.trim() && novaCoordNome.trim()) {
      try {
        await fetch("/api/auth", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "invite", nome: novaCoordNome, email: novaCoordEmail, papel: "coordenadora", escola_id: er.data.id, permissoes: [] }) });
      } catch(e) {}
    }
    setNovaEscola(""); setNovoLimite(10); setNovaCoordEmail(""); setNovaCoordNome(""); setShowNew(false); setCreating(false);
    setMsg(er.data ? "Escola criada!" : "Erro ao criar"); loadAll();
  }

  async function toggleEscola(escola: any) {
    await supabase.from("escolas").update({ ativa: !escola.ativa }).eq("id", escola.id);
    setMsg(escola.ativa ? "Escola desativada" : "Escola ativada"); loadAll();
  }

  async function salvarEscola() {
    if (!editEscola) return;
    await supabase.from("escolas").update({ nome: editNome, limite_usuarios: editLimite }).eq("id", editEscola.id);
    setEditEscola(null); setMsg("Escola atualizada!"); loadAll();
  }

  async function resetSenha(email: string) {
    try {
      var r = await fetch("/api/auth", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "reset_password", email: email }) });
      var d = await r.json();
      setMsg(d.success ? "Link enviado para " + email : "Erro: " + (d.error || ""));
    } catch(e) { setMsg("Erro de conexao"); }
  }
  ...
  function getUserCount(escolaId: string) { return allUsers.filter(function(u: any) { return u.escola_id === escolaId; }).length; }

  var totalEscolas = escolas.length;
  var ativas = escolas.filter(function(e: any) { return e.ativa; }).length;
  var totalUsers = allUsers.length;

  var saTabs = [{ id: "sa-dash", icon: "&#128200;", label: "Dashboard" },{ id: "sa-escolas", icon: "&#127979;", label: "Escolas" },{ id: "sa-users", icon: "&#128101;", label: "Usuarios" }];

  return (
    <div style={{ background: C.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      {msg && <Msg message={msg} onClose={function() { setMsg(""); }} />}
      {editEscola && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}><div style={{ background: C.white, borderRadius: 20, padding: 20, maxWidth: 380, width: "100%" }}>
        <h3 style={{ fontSize: 18, fontWeight: 700, color: C.text, margin: "0 0 16px" }}>Editar escola</h3>
        <label style={{ fontSize: 13, fontWeight: 600, color: C.text, display: "block", marginBottom: 6 }}>Nome</label>
        <input value={editNome} onChange={function(e) { setEditNome(e.target.value); }} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + C.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 10 }} />
        <label style={{ fontSize: 13, fontWeight: 600, color: C.text, display: "block", marginBottom: 6 }}>Limite de usuarios</label>
        <input type="number" value={editLimite} onChange={function(e) { setEditLimite(Number(e.target.value)); }} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + C.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 16 }} />
        <div style={{ display: "flex", gap: 8 }}><button onClick={function() { setEditEscola(null); }} style={{ flex: 1, padding: 10, borderRadius: 10, border: "2px solid " + C.border, background: "transparent", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancelar</button><button onClick={salvarEscola} style={{ flex: 1, padding: 10, borderRadius: 10, border: "none", background: C.blue, color: C.white, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Salvar</button></div>
      </div></div>)}
      <div style={{ position: "sticky", top: 0, zIndex: 150, background: C.blueDark, padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: C.white, fontSize: 16 }}>&#128203;</span></div><div><div style={{ fontSize: 15, fontWeight: 700, color: C.white }}>Super Admin</div><div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", letterSpacing: 1, textTransform: "uppercase" }}>KidFly</div></div></div>
      ...
      <div style={{ padding: "20px 20px 100px" }}>
        {ld && <p style={{ fontSize: 13, color: C.muted, textAlign: "center" }}>Carregando...</p>}
        {!ld && tab === "sa-dash" && (<div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: "0 0 20px" }}>Painel KidFly</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
            <div style={{ background: C.white, borderRadius: 16, padding: 16, border: "1px solid " + C.border, textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 700, color: C.blue }}>{totalEscolas}</div><div style={{ fontSize: 11, color: C.muted }}>Escolas</div></div>
            <div style={{ background: C.white, borderRadius: 16, padding: 16, border: "1px solid " + C.border, textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 700, color: C.green }}>{ativas}</div><div style={{ fontSize: 11, color: C.muted }}>Ativas</div></div>
            <div style={{ background: C.white, borderRadius: 16, padding: 16, border: "1px solid " + C.border, textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 700, color: C.pink }}>{totalUsers}</div><div style={{ fontSize: 11, color: C.muted }}>Usuarios</div></div>
          </div>
          <div style={{ background: C.white, borderRadius: 16, padding: 16, border: "1px solid " + C.border }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: C.text, margin: "0 0 12px" }}>Escolas</h3>
            {escolas.map(function(e: any) { var count = getUserCount(e.id); return (<div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderTop: "1px solid " + C.border }}>
              <div><div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{e.nome}</div><div style={{ fontSize: 12, color: C.muted }}>{count}/{e.limite_usuarios || "?"} usuarios</div></div>
              <span style={{ fontSize: 11, padding: "4px 8px", borderRadius: 6, fontWeight: 600, background: e.ativa ? C.greenLight : C.pinkLight, color: e.ativa ? C.green : C.pink }}>{e.ativa ? "Ativa" : "Inativa"}</span>
            </div>); })}
          </div>
        </div>)}
        {!ld && tab === "sa-escolas" && (<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: 0 }}>Escolas</h2>
            <button onClick={function() { setShowNew(!showNew); }} style={{ background: C.pink, color: C.white, border: "none", borderRadius: 12, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{showNew ? "X" : "+ Nova"}</button>
          </div>
          {showNew && (<div style={{ background: C.white, borderRadius: 16, padding: 16, marginBottom: 16, border: "1px solid " + C.border }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: C.text, display: "block", marginBottom: 6 }}>Nome da escola</label>
            <input value={novaEscola} onChange={function(e) { setNovaEscola(e.target.value); }} placeholder="Ex: Escola Pequenos Brilhantes" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + C.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 10 }} />
            <label style={{ fontSize: 13, fontWeight: 600, color: C.text, display: "block", marginBottom: 6 }}>Limite de usuarios</label>
            <input type="number" value={novoLimite} onChange={function(e) { setNovoLimite(Number(e.target.value)); }} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + C.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 12 }} />
            <div style={{ background: C.blueLight, borderRadius: 10, padding: 12, marginBottom: 12 }}>
              <label style={{ fontSize: 13, fontWeight: 600, color: C.blue, display: "block", marginBottom: 8 }}>Coordenadora (opcional - recebe convite)</label>
              <input value={novaCoordNome} onChange={function(e) { setNovaCoordNome(e.target.value); }} placeholder="Nome da coordenadora" style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "2px solid " + C.border, fontSize: 13, outline: "none", boxSizing: "border-box", marginBottom: 8 }} />
              <input type="email" value={novaCoordEmail} onChange={function(e) { setNovaCoordEmail(e.target.value); }} placeholder="E-mail da coordenadora" style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "2px solid " + C.border, fontSize: 13, outline: "none", boxSizing: "border-box" }} />
            </div>
            <button onClick={criarEscola} disabled={creating} style={{ width: "100%", padding: 10, borderRadius: 10, border: "none", background: C.green, color: C.white, fontSize: 14, fontWeight: 600, cursor: "pointer", opacity: creating ? 0.7 : 1 }}>{creating ? "Criando..." : "Criar escola"}</button>
          </div>)}
          {escolas.map(function(e: any) { var count = getUserCount(e.id); return (<div key={e.id} style={{ background: C.white, borderRadius: 16, padding: 16, marginBottom: 12, border: "1px solid " + C.border }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div><h3 style={{ fontSize: 15, fontWeight: 600, color: C.text, margin: "0 0 2px" }}>{e.nome}</h3><span style={{ fontSize: 12, color: C.muted }}>{count}/{e.limite_usuarios || "?"} usuarios</span></div>
              <span style={{ fontSize: 11, padding: "4px 8px", borderRadius: 6, fontWeight: 600, background: e.ativa ? C.greenLight : C.pinkLight, color: e.ativa ? C.green : C.pink }}>{e.ativa ? "Ativa" : "Inativa"}</span>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={function() { setEditEscola(e); setEditNome(e.nome); setEditLimite(e.limite_usuarios || 10); }} style={{ flex: 1, padding: "6px", borderRadius: 8, border: "none", background: C.blueLight, fontSize: 12, fontWeight: 600, color: C.blue, cursor: "pointer" }}>&#9998; Editar</button>
              <button onClick={function() { toggleEscola(e); }} style={{ flex: 1, padding: "6px", borderRadius: 8, border: "none", background: e.ativa ? C.yellowLight : C.greenLight, fontSize: 12, fontWeight: 600, color: e.ativa ? "#B8860B" : C.green, cursor: "pointer" }}>{e.ativa ? "Desativar" : "Ativar"}</button>
            </div>
          </div>); })}
        </div>)}
        {!ld && tab === "sa-users" && (<div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: "0 0 20px" }}>Usuarios ({allUsers.length})</h2>
          {allUsers.map(function(u: any) { return (<div key={u.id} style={{ background: C.white, borderRadius: 14, padding: 14, marginBottom: 10, border: "1px solid " + C.border }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div><div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{u.nome}</div><div style={{ fontSize: 12, color: C.muted }}>{u.email}</div></div>
              <span style={{ fontSize: 11, padding: "4px 8px", borderRadius: 6, fontWeight: 600, background: C.blueLight, color: C.blue }}>{u.papel}</span>
            </div>
            <div style={{ fontSize: 11, color: C.muted, marginBottom: 8 }}>{u.escolas?.nome || "Sem escola"}</div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={function() { resetSenha(u.email); }} style={{ flex: 1, padding: "5px", borderRadius: 8, border: "1px solid " + C.border, background: C.white, fontSize: 11, fontWeight: 600, color: C.blue, cursor: "pointer" }}>Resetar senha</button>
              <button onClick={function() { removeUser(u.id); }} style={{ flex: 1, padding: "5px", borderRadius: 8, border: "none", background: C.pinkLight, fontSize: 11, fontWeight: 600, color: C.pink, cursor: "pointer" }}>Remover</button>
            </div>
          </div>); })}
        </div>)}
      </div>
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: C.blueDark, display: "flex", justifyContent: "space-around", padding: "6px 4px 20px", zIndex: 50 }}>
        {saTabs.map(function(t) { return (<button key={t.id} onClick={function() { setTab(t.id); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "none", border: "none", padding: "4px 6px", cursor: "pointer", opacity: tab === t.id ? 1 : 0.5 }}><span style={{ fontSize: 20, display: "block" }} dangerouslySetInnerHTML={{ __html: t.icon }} /><span style={{ fontSize: 10, fontWeight: tab === t.id ? 700 : 500, color: tab === t.id ? C.white : "rgba(255,255,255,0.5)" }}>{t.label}</span>{tab === t.id && <div style={{ width: 4, height: 4, borderRadius: "50%", background: C.pink, marginTop: 1 }} />}</button>); })}
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  function handleLogin(ud: any) { setUser(ud); if (ud.papel !== "superadmin") { loadData(ud); } }
