  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: COLORS.white, borderRadius: 20, padding: "32px 24px", textAlign: "center", maxWidth: 320, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <p style={{ fontSize: 13, color: COLORS.textMuted, margin: "0 0 20px" }}>Operacao realizada com sucesso.</p>
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: COLORS.white, borderRadius: 20, padding: 20, maxWidth: 380, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ marginTop: 16, marginBottom: 12 }}>
          ...
          {aviso.data_evento ? (
            <div style={{ fontSize: 14, color: COLORS.blue, fontWeight: 600, background: COLORS.blueLight, padding: "8px 12px", borderRadius: 10, display: "inline-block" }}>
              &#128197; {new Date(aviso.data_evento + "T12:00:00").toLocaleDateString("pt-BR")}
            </div>
          ) : (
            <div style={{ fontSize: 13, color: COLORS.textMuted }}>Sem data definida</div>
          )}
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {isTodos ? (
            <div style={{ fontSize: 13, color: COLORS.green, fontWeight: 600, background: COLORS.greenLight, padding: "8px 12px", borderRadius: 10, display: "inline-block" }}>&#128101; Todos da escola</div>
          ) : assignedUsers.length > 0 ? (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {assignedUsers.map(function(name: string, i: number) {
                return <span key={i} style={{ fontSize: 12, background: COLORS.blueLight, color: COLORS.blue, padding: "4px 10px", borderRadius: 8, fontWeight: 600 }}>{name}</span>;
              })}
            </div>
          ) : (
            <div style={{ fontSize: 13, color: COLORS.textMuted }}>Nenhum responsavel atribuido</div>
          )}
        </div>

        <div style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 16 }}>Publicado em {new Date(aviso.created_at).toLocaleDateString("pt-BR")}</div>
        ...
        {planos.slice(0, 3).map(function(p: any) {
          return (
            <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderTop: "1px solid " + COLORS.border }}>
              <div><div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text }}>{p.titulo}</div><div style={{ fontSize: 12, color: COLORS.textMuted }}>{p.turmas?.nome}</div></div>
        ...
        {allEventos.slice(0, 4).map(function(ev: any) {
          return (
            <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderTop: "1px solid " + COLORS.border }}>
              ...
              <div><div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text }}>{ev.titulo}</div><div style={{ fontSize: 12, color: COLORS.textMuted }}>{new Date(ev.data + "T12:00:00").toLocaleDateString("pt-BR")}</div></div>
            </div>
          );
        })}
      </div>
...
function MuralScreen({ avisos, user, usuarios, destinatarios, onAvisoCreated, onAvisoUpdated, onAvisoDeleted }: any) {
  ...
  async function criarAviso() {
    ...
    if (res.data) {
      ...
      if (atribuicao === "todos") {
        ...
        var dests = selectedUsers.map(function(uid) { return { aviso_id: avisoId, usuario_id: uid, todos: false }; });
        if (dests.length > 0) await supabase.from("aviso_destinatarios").insert(dests);
      }
  ...
  function startEdit(aviso: any) {
    ...
    var destInfo = destinatarios.filter(function(d: any) { return d.aviso_id === aviso.id; });
    var hasTodos = destInfo.some(function(d: any) { return d.todos; });
    if (hasTodos) { setEditAtribuicao("todos"); setEditSelectedUsers([]); }
    else {
      setEditAtribuicao("selecionar");
      setEditSelectedUsers(destInfo.filter(function(d: any) { return d.usuario_id; }).map(function(d: any) { return d.usuario_id; }));
    }
  }
  ...
  async function salvarEdicao() {
    ...
    if (editAtribuicao === "todos") {
      await supabase.from("aviso_destinatarios").insert({ aviso_id: editingAviso.id, todos: true });
    } else {
      var dests = editSelectedUsers.map(function(uid) { return { aviso_id: editingAviso.id, usuario_id: uid, todos: false }; });
      if (dests.length > 0) await supabase.from("aviso_destinatarios").insert(dests);
    }
    ...
    if (onAvisoUpdated) onAvisoUpdated();
  }
  ...
  async function excluirAviso() {
    ...
    setLista(lista.filter(function(a: any) { return a.id !== editingAviso.id; }));
    setEditingAviso(null);
    setSuccessMsg("Aviso excluido com sucesso!");
    if (onAvisoDeleted) onAvisoDeleted();
  }
  ...
  function toggleEditUser(uid: string) {
    if (editSelectedUsers.includes(uid)) { setEditSelectedUsers(editSelectedUsers.filter(function(id) { return id !== uid; })); }
    else { setEditSelectedUsers([...editSelectedUsers, uid]); }
  }
  ...
  return (
    <div>
      ...
      {editingAviso && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: COLORS.white, borderRadius: 20, padding: 20, maxWidth: 380, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", maxHeight: "85vh", overflowY: "auto" }}>
            ...
            {editAtribuicao === "selecionar" && (
              <div style={{ marginBottom: 12, maxHeight: 120, overflowY: "auto", border: "1px solid " + COLORS.border, borderRadius: 10, padding: 8 }}>
                {outrosUsuarios.map(function(u: any) {
                  var selected = editSelectedUsers.includes(u.id);
                  return (
                    <div key={u.id} onClick={function() { toggleEditUser(u.id); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 4px", cursor: "pointer", borderBottom: "1px solid " + COLORS.border }}>
                      ...
                      <div><div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text }}>{u.nome}</div><div style={{ fontSize: 11, color: COLORS.textMuted }}>{u.papel}</div></div>
                    </div>
                  );
                })}
              </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        {isEditor && <button onClick={function() { setMostrarForm(!mostrarForm); }} style={{ background: COLORS.pink, color: COLORS.white, border: "none", borderRadius: 12, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      {lista.map(function(aviso: any) {
        return (
          <div key={aviso.id} style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + (aviso.urgente ? COLORS.pink + "33" : COLORS.border), borderLeft: aviso.urgente ? "4px solid " + COLORS.pink : "4px solid " + COLORS.border }}>
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto" }}>
        {[{ key: "todos", label: "Todos" }, { key: "pendente", label: "Pendentes" }, { key: "aprovado", label: "Aprovados" }, { key: "revisar", label: "Revisar" }].map(function(f) {
          return <button key={f.key} onClick={function() { setFilter(f.key); }} style={{ padding: "8px 16px", borderRadius: 20, border: "none", background: filter === f.key ? COLORS.blue : COLORS.blueLight, color: filter === f.key ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>{f.label}</button>;
        })}
      </div>
        ...
        return (
          <div key={p.id} style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div><h3 style={{ fontSize: 15, fontWeight: 600, color: COLORS.text, margin: "0 0 2px" }}>{p.titulo}</h3><span style={{ fontSize: 13, color: COLORS.textMuted }}>{p.turmas?.nome}</span></div>
              <StatusBadge status={p.status} />
            </div>
            {p.feedback && p.status === "revisar" && (
              <div style={{ background: COLORS.yellowLight, borderRadius: 10, padding: "8px 12px", marginTop: 8 }}>
      ...
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, textAlign: "center" }}>
          ...
          {Array.from({ length: daysInMonth }, function(_, i) {
            ...
            return <div key={day} style={{ padding: "6px 2px", borderRadius: 8, fontSize: 13, fontWeight: isToday ? 700 : 400, background: isToday ? COLORS.blue : hasEvent ? COLORS.pinkLight : "transparent", color: isToday ? COLORS.white : hasEvent ? COLORS.pink : COLORS.text }}>{day}</div>;
          })}
        </div>
      </div>
    ...
    <div>
      ...
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
          <div><h3 style={{ fontSize: 15, fontWeight: 600, color: COLORS.text, margin: "0 0 2px" }}>Visita - Bercario II</h3><span style={{ fontSize: 12, color: COLORS.textMuted }}>24/03/2026</span></div>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: COLORS.blueLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: COLORS.blue }}>{items.filter(function(i) { return i.checked; }).length}/{items.length}</div>
        </div>
        {items.map(function(item) {
          return (
            <div key={item.id} onClick={function() { setItems(items.map(function(i) { return i.id === item.id ? Object.assign({}, i, { checked: !i.checked }) : i; })); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderTop: "1px solid " + COLORS.border, cursor: "pointer" }}>
              <div style={{ width: 24, height: 24, borderRadius: 8, border: "2px solid " + (item.checked ? COLORS.green : COLORS.border), background: item.checked ? COLORS.green : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{item.checked && <span style={{ color: COLORS.white, fontSize: 14, fontWeight: 700 }}>&#10003;</span>}</div>
...
function GaleriaScreen() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto" }}>
        {["Todas", "Bercario II", "Maternal I", "Jardim I"].map(function(t, i) { return <button key={t} style={{ padding: "8px 16px", borderRadius: 20, border: "none", background: i === 0 ? COLORS.blue : COLORS.blueLight, color: i === 0 ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>{t}</button>; })}
      </div>
        ...
        {fotos.map(function(foto) {
          return (
            <div key={foto.id} style={{ borderRadius: 14, overflow: "hidden", background: COLORS.white, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}>
              ...
              <div style={{ padding: 10 }}><div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text }}>{foto.desc}</div><div style={{ fontSize: 11, color: COLORS.textMuted }}>{foto.turma}</div></div>
            </div>
          );
        })}
      </div>
...
export default function Home() {
  ...
  async function loadData(userData: any) {
    ...
    setDestinatarios(results[5].data || []);
    setLoading(false);
  }
  ...
  function reloadDestinatarios() {
    supabase.from("aviso_destinatarios").select("*").then(function(res) { setDestinatarios(res.data || []); });
  }
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: COLORS.bg }}>
      <div style={{ textAlign: "center" }}><div style={{ fontSize: 40, marginBottom: 12 }}>&#128203;</div><p style={{ color: COLORS.textMuted, fontSize: 14 }}>Carregando...</p></div>
    </div>
  );
    ...
    <div style={{ background: COLORS.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: COLORS.white, borderBottom: "1px solid " + COLORS.border, padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, " + COLORS.blue + ", " + COLORS.blueDark + ")", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: COLORS.white, fontSize: 16 }}>&#128203;</span></div>
          <div><div style={{ fontSize: 15, fontWeight: 700, color: COLORS.text }}>Coordinator</div><div style={{ fontSize: 10, color: COLORS.textMuted, letterSpacing: 1, textTransform: "uppercase" }}>by KidFly</div></div>
        </div>
