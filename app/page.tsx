...
function GestaoScreen({ user, turmas, usuarios, perms, onUpdate }: any) {
  ...
  async function delUser(id: string) { await supabase.from("permissoes_usuario").delete().eq("usuario_id", id); await supabase.from("usuarios").delete().eq("id", id); setMsg("Removido!"); onUpdate(); }
...
function GaleriaScreen({ user, turmas, perms }: any) {
  ...
  var turmaIds = turmas.map(function(t: any) { return t.id; });
  useEffect(function() { setLd(true); supabase.from("fotos").select("*, turmas(nome)").order("created_at", { ascending: false }).then(function(r) { var filtered = (r.data || []).filter(function(f: any) { return turmaIds.includes(f.turma_id); }); setFotos(filtered); setLd(false); }); }, []);
...
function SuperAdminPanel({ user, onLogout }: any) {
  var _a = useState("dashboard"); var saTab = _a[0]; var setSaTab = _a[1];
  ...
  var _g = useState(""); var newEscolaNome = _g[0]; var setNewEscolaNome = _g[1];
  var _h = useState(""); var newCoordNome = _h[0]; var setNewCoordNome = _h[1];
  var _i = useState(""); var newCoordEmail = _i[0]; var setNewCoordEmail = _i[1];
  var _j = useState(false); var creating = _j[0]; var setCreating = _j[1];
  var _k = useState<any>(null); var confirmDel = _k[0]; var setConfirmDel = _k[1];
  ...
  async function loadSA() {
    ...
    setEscolas(r[0].data || []); setAllUsers(r[1].data || []); setLd(false);
  }

  useEffect(function() { loadSA(); }, []);
  ...
  async function toggleEscola(id: string, ativa: boolean) {
    ...
    setMsg(ativa ? "Escola ativada!" : "Escola desativada!");
  }

  async function deleteEscola(escola: any) {
    var escolaUsers = allUsers.filter(function(u: any) { return u.escola_id === escola.id; });
    for (var i = 0; i < escolaUsers.length; i++) {
      await supabase.from("permissoes_usuario").delete().eq("usuario_id", escolaUsers[i].id);
    }
    await supabase.from("usuarios").delete().eq("escola_id", escola.id);
    await supabase.from("turmas").delete().eq("escola_id", escola.id);
    await supabase.from("avisos").delete().eq("escola_id", escola.id);
    await supabase.from("eventos").delete().eq("escola_id", escola.id);
    await supabase.from("checklist_modelo").delete().eq("escola_id", escola.id);
    await supabase.from("escolas").delete().eq("id", escola.id);
    setEscolas(escolas.filter(function(e: any) { return e.id !== escola.id; }));
    setAllUsers(allUsers.filter(function(u: any) { return u.escola_id !== escola.id; }));
    setConfirmDel(null); setMsg("Escola excluida permanentemente!");
  }

  async function criarEscola() {
    if (!newEscolaNome.trim()) return;
    setCreating(true);
    var er = await supabase.from("escolas").insert({ nome: newEscolaNome, ativa: true }).select().single();
    if (er.data) {
      if (newCoordNome.trim() && newCoordEmail.trim()) {
        try {
          var res = await fetch("/api/auth", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "invite", nome: newCoordNome, email: newCoordEmail, papel: "coordenadora", escola_id: er.data.id, permissoes: [] }) });
          var data = await res.json();
          if (data.success) { setMsg("Escola criada e convite enviado para " + newCoordEmail + "!"); }
          else { setMsg("Escola criada! Erro no convite: " + (data.error || "")); }
        } catch (e) { setMsg("Escola criada! Erro ao enviar convite."); }
      } else { setMsg("Escola criada com sucesso!"); }
      setNewEscolaNome(""); setNewCoordNome(""); setNewCoordEmail(""); setShowNewEscola(false); loadSA();
    }
    setCreating(false);
  }
  ...
  async function deleteUser(id: string) {
    ...
    setAllUsers(allUsers.filter(function(u: any) { return u.id !== id; })); setMsg("Usuario removido!");
  }

  var totalEscolas = escolas.length; var ativas = escolas.filter(function(e: any) { return e.ativa; }).length; var totalUsers = allUsers.filter(function(u: any) { return u.papel !== "superadmin"; }).length;

  return (
    <div style={{ background: C.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto" }}>
      {msg && <Msg message={msg} onClose={function() { setMsg(""); }} />}

      {confirmDel && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}><div style={{ background: C.white, borderRadius: 20, padding: "24px 20px", maxWidth: 340, width: "100%", textAlign: "center" }}>
        <div style={{ fontSize: 40, marginBottom: 8 }}>&#9888;&#65039;</div>
        <h3 style={{ fontSize: 18, fontWeight: 700, color: C.text, margin: "0 0 8px" }}>Excluir escola?</h3>
        <p style={{ fontSize: 13, color: C.muted, margin: "0 0 20px" }}>Todos os dados de <strong>{confirmDel.nome}</strong> serao excluidos permanentemente: turmas, avisos, planos, fotos, usuarios e mensagens. Essa acao nao pode ser desfeita.</p>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={function() { setConfirmDel(null); }} style={{ flex: 1, padding: 12, borderRadius: 12, border: "2px solid " + C.border, background: "transparent", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancelar</button>
          <button onClick={function() { deleteEscola(confirmDel); }} style={{ flex: 1, padding: 12, borderRadius: 12, border: "none", background: C.pink, color: C.white, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Excluir</button>
        </div>
      </div></div>)}
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 150, background: C.blueDark, padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div onClick={onLogout} style={{ padding: "6px 14px", borderRadius: 8, background: "rgba(255,255,255,0.15)", color: C.white, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Sair</div>
      </div>

      <div style={{ display: "flex", gap: 4, padding: "12px 20px 0" }}>
        {[{ id: "dashboard", l: "Painel" },{ id: "escolas", l: "Escolas" },{ id: "usuarios", l: "Usuarios" }].map(function(t) { return <button key={t.id} onClick={function() { setSaTab(t.id); }} style={{ flex: 1, padding: "10px 8px", borderRadius: 12, border: "none", background: saTab === t.id ? C.blue : C.blueLight, color: saTab === t.id ? C.white : C.blue, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{t.l}</button>; })}
      </div>

      <div style={{ padding: 20 }}>
        {ld && <p style={{ fontSize: 13, color: C.muted, textAlign: "center" }}>Carregando...</p>}

        {!ld && saTab === "dashboard" && (<div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: "0 0 20px" }}>Painel KidFly</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
            {[{ l: "Escolas", v: String(totalEscolas), bg: C.blueLight, cl: C.blue },{ l: "Ativas", v: String(ativas), bg: C.greenLight, cl: C.green },{ l: "Usuarios", v: String(totalUsers), bg: C.pinkLight, cl: C.pink }].map(function(s, i) { return (<div key={i} style={{ background: C.white, borderRadius: 16, padding: 16, border: "1px solid " + C.border, textAlign: "center" }}><div style={{ fontSize: 24, fontWeight: 700, color: s.cl }}>{s.v}</div><div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>{s.l}</div></div>); })}
          </div>
          <div style={{ background: C.white, borderRadius: 16, padding: 16, border: "1px solid " + C.border }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: C.text, margin: "0 0 12px" }}>Escolas recentes</h3>
            {escolas.slice(0, 5).map(function(e: any) { return (<div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderTop: "1px solid " + C.border }}>
              <div><div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{e.nome}</div></div>
              <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: 6, fontWeight: 600, background: e.ativa ? C.greenLight : C.pinkLight, color: e.ativa ? C.green : C.pink }}>{e.ativa ? "Ativa" : "Inativa"}</span>
            </div>); })}
          </div>
        </div>)}

        {!ld && saTab === "escolas" && (<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: 0 }}>Escolas ({totalEscolas})</h2>
            <button onClick={function() { setShowNewEscola(!showNewEscola); }} style={{ background: C.blue, color: C.white, border: "none", borderRadius: 12, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{showNewEscola ? "X" : "+ Nova"}</button>
          </div>
          {showNewEscola && (<div style={{ background: C.white, borderRadius: 16, padding: 16, border: "1px solid " + C.border, marginBottom: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: C.text, display: "block", marginBottom: 6 }}>Nome da escola</label>
            <input value={newEscolaNome} onChange={function(e) { setNewEscolaNome(e.target.value); }} placeholder="Ex: Escola Pequenos Brilhantes" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + C.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 12 }} />
            <label style={{ fontSize: 13, fontWeight: 600, color: C.text, display: "block", marginBottom: 6 }}>Coordenadora (opcional)</label>
            <input value={newCoordNome} onChange={function(e) { setNewCoordNome(e.target.value); }} placeholder="Nome da coordenadora" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + C.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 8 }} />
            <input type="email" value={newCoordEmail} onChange={function(e) { setNewCoordEmail(e.target.value); }} placeholder="Email da coordenadora" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + C.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 12 }} />
          ...
          {escolas.map(function(e: any) {
            ...
            return (<div key={e.id} style={{ background: C.white, borderRadius: 16, padding: 16, marginBottom: 12, border: "1px solid " + C.border }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <div><h3 style={{ fontSize: 15, fontWeight: 600, color: C.text, margin: "0 0 2px" }}>{e.nome}</h3><span style={{ fontSize: 12, color: C.muted }}>{usersCount} usuarios</span></div>
                <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: 6, fontWeight: 600, background: e.ativa ? C.greenLight : C.pinkLight, color: e.ativa ? C.green : C.pink }}>{e.ativa ? "Ativa" : "Inativa"}</span>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                {e.ativa ? <button onClick={function() { toggleEscola(e.id, false); }} style={{ flex: 1, padding: "8px", borderRadius: 8, border: "2px solid " + C.yellow, background: "transparent", color: "#B8860B", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Desativar</button> : <button onClick={function() { toggleEscola(e.id, true); }} style={{ flex: 1, padding: "8px", borderRadius: 8, border: "none", background: C.green, color: C.white, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Ativar</button>}
                <button onClick={function() { setConfirmDel(e); }} style={{ padding: "8px 12px", borderRadius: 8, border: "none", background: C.pink, color: C.white, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>&#128465; Excluir</button>
              </div>
            </div>);
          })}
        </div>)}

        {!ld && saTab === "usuarios" && (<div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: "0 0 20px" }}>Usuarios ({totalUsers})</h2>
          {allUsers.filter(function(u: any) { return u.papel !== "superadmin"; }).map(function(u: any) { return (<div key={u.id} style={{ background: C.white, borderRadius: 16, padding: 16, marginBottom: 10, border: "1px solid " + C.border }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div><div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{u.nome}</div><div style={{ fontSize: 12, color: C.muted }}>{u.email}</div><div style={{ display: "flex", gap: 6, marginTop: 4 }}><span style={{ fontSize: 11, background: C.blueLight, color: C.blue, padding: "2px 6px", borderRadius: 4, fontWeight: 600 }}>{u.papel}</span><span style={{ fontSize: 11, background: C.yellowLight, color: "#B8860B", padding: "2px 6px", borderRadius: 4, fontWeight: 600 }}>{u.escolas?.nome || "Sem escola"}</span></div></div>
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
              <button onClick={function() { resetSenha(u.email); }} style={{ flex: 1, padding: "6px", borderRadius: 8, border: "1px solid " + C.border, background: C.white, fontSize: 11, fontWeight: 600, color: C.blue, cursor: "pointer" }}>Resetar senha</button>
              <button onClick={function() { deleteUser(u.id); }} style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: C.pinkLight, fontSize: 11, fontWeight: 600, color: C.pink, cursor: "pointer" }}>Remover</button>
            </div>
          </div>); })}
        </div>)}
      </div>
...
export default function Home() {
  ...
  useEffect(function() {
    supabase.auth.onAuthStateChange(function(event: string) {
      if (event === "PASSWORD_RECOVERY") { setShowPwForm(true); }
    });
  }, []);

  async function loadData(ud: any) { setLd(true); var r = await Promise.all([supabase.from("avisos").select("*").eq("escola_id", ud.escola_id).order("created_at", { ascending: false }), supabase.from("planos").select("*, turmas!inner(nome, escola_id)").eq("turmas.escola_id", ud.escola_id).order("created_at", { ascending: false }), supabase.from("eventos").select("*").eq("escola_id", ud.escola_id).order("data", { ascending: true }), supabase.from("turmas").select("*, usuarios(nome)").eq("escola_id", ud.escola_id), supabase.from("usuarios").select("*").eq("escola_id", ud.escola_id), supabase.from("aviso_destinatarios").select("*"), supabase.from("permissoes_usuario").select("*").eq("usuario_id", ud.id)]); setAvisos(r[0].data || []); setPlanos(r[1].data || []); setEventos(r[2].data || []); setTurmas(r[3].data || []); setUsuarios(r[4].data || []); var avisoIds = (r[0].data || []).map(function(a: any) { return a.id; }); setDest((r[5].data || []).filter(function(d: any) { return avisoIds.includes(d.aviso_id); })); setPerms(r[6].data || []); setLd(false); }
