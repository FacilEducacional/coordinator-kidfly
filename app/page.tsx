    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ textAlign: "center", fontSize: 13, color: COLORS.textMuted, marginTop: 16, marginBottom: 0 }}>
          Acesso exclusivo para assinantes KidFly Equipe e Escola
        </p>
...
function DashboardScreen({ user, avisos, planos, eventos, turmas }: any) {
  ...
  const allEventos = [
    ...eventos,
    ...avisos.filter((a: any) => a.data_evento).map((a: any) => ({
      id: "aviso-" + a.id,
      titulo: a.titulo,
      data: a.data_evento,
      cor: a.urgente ? COLORS.pink : COLORS.blue,
    })),
  ...
  return (
    <div>
      ...
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}>
        ...
        {allEventos.slice(0, 4).map((ev: any) => (
...
function MuralScreen({ avisos, user, usuarios, onAvisoCreated }: any) {
  ...
  const [atribuicao, setAtribuicao] = useState("todos");
  ...
  async function criarAviso() {
    ...
    if (dataEvento) {
      insertData.data_evento = dataEvento;
    }
    ...
    if (res.data) {
      ...
      if (atribuicao === "todos") {
        ...
        var destinatarios = selectedUsers.map(function(uid) {
          return { aviso_id: avisoId, usuario_id: uid, todos: false };
        });
        if (destinatarios.length > 0) {
          await supabase.from("aviso_destinatarios").insert(destinatarios);
        }
      }
  ...
  function toggleUser(uid: string) {
    if (selectedUsers.includes(uid)) {
      setSelectedUsers(selectedUsers.filter(function(id) { return id !== uid; }));
    } else {
    ...
    <div>
      ...
      {mostrarForm && (
        <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 16, border: "1px solid " + COLORS.border }}>
          <input value={titulo} onChange={function(e) { setTitulo(e.target.value); }} placeholder="Titulo do aviso ou evento..."
          ...
          <input type="date" value={dataEvento} onChange={function(e) { setDataEvento(e.target.value); }}
            style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "2px solid " + COLORS.border, fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 12 }}
          />

          <label style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, display: "block", marginBottom: 8 }}>Atribuir a:</label>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button onClick={function() { setAtribuicao("todos"); setSelectedUsers([]); }}
              style={{ flex: 1, padding: "8px", borderRadius: 10, border: "none", background: atribuicao === "todos" ? COLORS.blue : COLORS.blueLight, color: atribuicao === "todos" ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              Todos
            </button>
            <button onClick={function() { setAtribuicao("selecionar"); }}
              style={{ flex: 1, padding: "8px", borderRadius: 10, border: "none", background: atribuicao === "selecionar" ? COLORS.blue : COLORS.blueLight, color: atribuicao === "selecionar" ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              Selecionar
            </button>
          </div>
          ...
          {atribuicao === "selecionar" && (
            <div style={{ marginBottom: 12, maxHeight: 150, overflowY: "auto", border: "1px solid " + COLORS.border, borderRadius: 10, padding: 8 }}>
              {outrosUsuarios.map(function(u: any) {
                ...
                return (
                  <div key={u.id} onClick={function() { toggleUser(u.id); }}
                    style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 4px", cursor: "pointer", borderBottom: "1px solid " + COLORS.border }}>
                    <div style={{
...
function PlanosScreen({ planos: initialPlanos }: any) {
  ...
  async function updateStatus(id: string, status: string) {
    await supabase.from("planos").update({ status: status }).eq("id", id);
    setPlanos(planos.map(function(p: any) { return p.id === id ? Object.assign({}, p, { status: status }) : p; }));
  }
          ...
          <div key={p.id} style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}>
            ...
            {p.status === "pendente" && (
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                ...
                <button onClick={function() { updateStatus(p.id, "revisar"); }} style={{ flex: 1, padding: "8px", borderRadius: 10, border: "2px solid " + COLORS.yellow, background: "transparent", color: "#B8860B", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>&#8635; Pedir ajuste</button>
              </div>
            )}
          </div>
        );
  ...
  return (
    <div>
      ...
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border, marginBottom: 20 }}>
        <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.text, margin: "0 0 12px" }}>{monthNames[currentMonth]} {currentYear}</h3>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, textAlign: "center" }}>
          ...
          {Array.from({ length: firstDay }, function(_, i) {
      ...
      <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.text, margin: "0 0 12px" }}>Todos os eventos</h3>
      {allEventos.length === 0 && <p style={{ fontSize: 13, color: COLORS.textMuted }}>Nenhum evento</p>}
      {allEventos.map(function(ev: any) {
        return (
  ...
  return (
    <div>
      ...
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}>
        ...
        <textarea placeholder="Observacoes gerais sobre a visita..."
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: COLORS.white, borderBottom: "1px solid " + COLORS.border, padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div onClick={function() { setUser(null); setActiveTab("dashboard"); }} style={{ width: 36, height: 36, borderRadius: 10, background: COLORS.pinkLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, cursor: "pointer" }}>
          &#128100;
        </div>
      </div>
