...
function SuccessMessage({ message, onClose }: { message: string; onClose: () => void }) {
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: COLORS.white, borderRadius: 20, padding: "32px 24px", textAlign: "center", maxWidth: 320, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>&#9989;</div>
        <h3 style={{ fontSize: 18, fontWeight: 700, color: COLORS.text, margin: "0 0 8px" }}>{message}</h3>
        <button onClick={onClose} style={{ width: "100%", padding: 12, borderRadius: 12, border: "none", background: COLORS.blue, color: COLORS.white, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Ok, entendi</button>
      </div>
    </div>
  );
}
...
function ViewEventModal({ aviso, destinatarios, usuarios, onClose }: any) {
  ...
  var assignedUsers = destInfo.filter(function(d: any) { return d.usuario_id; }).map(function(d: any) {
    var u = usuarios.find(function(u2: any) { return u2.id === d.usuario_id; });
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: COLORS.white, borderRadius: 20, padding: 20, maxWidth: 380, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ marginTop: 16, marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 4 }}>Data do evento</div>
          {aviso.data_evento ? <div style={{ fontSize: 14, color: COLORS.blue, fontWeight: 600, background: COLORS.blueLight, padding: "8px 12px", borderRadius: 10, display: "inline-block" }}>&#128197; {new Date(aviso.data_evento + "T12:00:00").toLocaleDateString("pt-BR")}</div> : <div style={{ fontSize: 13, color: COLORS.textMuted }}>Sem data definida</div>}
        </div>
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 4 }}>Responsaveis</div>
          {isTodos ? <div style={{ fontSize: 13, color: COLORS.green, fontWeight: 600, background: COLORS.greenLight, padding: "8px 12px", borderRadius: 10, display: "inline-block" }}>&#128101; Todos da escola</div> : assignedUsers.length > 0 ? <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{assignedUsers.map(function(name: string, i: number) { return <span key={i} style={{ fontSize: 12, background: COLORS.blueLight, color: COLORS.blue, padding: "4px 10px", borderRadius: 8, fontWeight: 600 }}>{name}</span>; })}</div> : <div style={{ fontSize: 13, color: COLORS.textMuted }}>Nenhum responsavel atribuido</div>}
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(145deg, " + COLORS.blue + " 0%, " + COLORS.blueDark + " 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        <div style={{ width: 72, height: 72, borderRadius: 20, background: "rgba(255,255,255,0.15)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", border: "1px solid rgba(255,255,255,0.2)" }}><span style={{ fontSize: 36 }}>&#128203;</span></div>
...
function DashboardScreen({ user, avisos, planos, eventos, turmas }: any) {
  ...
  var allEventos = [...eventos, ...avisos.filter(function(a: any) { return a.data_evento; }).map(function(a: any) { return { id: "aviso-" + a.id, titulo: a.titulo, data: a.data_evento, cor: a.urgente ? COLORS.pink : COLORS.blue }; })].sort(function(a: any, b: any) { return a.data > b.data ? 1 : -1; });
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
        {[{ label: "Planos pendentes", value: String(pendentes), icon: "&#128221;", bg: COLORS.yellowLight },{ label: "Avisos ativos", value: String(avisos.length), icon: "&#128226;", bg: COLORS.pinkLight },{ label: "Eventos", value: String(allEventos.length), icon: "&#128197;", bg: COLORS.blueLight },{ label: "Professoras", value: String(turmas.length), icon: "&#128105;&#8205;&#127979;", bg: COLORS.greenLight }].map(function(stat, i) {
          return (<div key={i} style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}><div style={{ width: 40, height: 40, borderRadius: 12, background: stat.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, marginBottom: 10 }} dangerouslySetInnerHTML={{ __html: stat.icon }} /><div style={{ fontSize: 24, fontWeight: 700, color: COLORS.text }}>{stat.value}</div><div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 2 }}>{stat.label}</div></div>);
        })}
      </div>
...
function MuralScreen({ avisos, user, usuarios, destinatarios, onAvisoCreated, onAvisoUpdated, onAvisoDeleted }: any) {
  ...
  var _s13 = useState("todos"); var editAtribuicao = _s13[0]; var setEditAtribuicao = _s13[1];
  var _s14 = useState<string[]>([]); var editSelectedUsers = _s14[0]; var setEditSelectedUsers = _s14[1];
  ...
  async function criarAviso() {
    ...
    if (res.data) {
      ...
      else { var dests = selectedUsers.map(function(uid) { return { aviso_id: res.data.id, usuario_id: uid, todos: false }; }); if (dests.length > 0) await supabase.from("aviso_destinatarios").insert(dests); }
      setLista([res.data, ...lista]); setTitulo(""); setDataEvento(""); setUrgente(false); setAtribuicao("todos"); setSelectedUsers([]); setMostrarForm(false);
      setSuccessMsg("Aviso publicado com sucesso!"); if (onAvisoCreated) onAvisoCreated(res.data);
    }
  }
  function startEdit(aviso: any) {
    ...
    var destInfo = destinatarios.filter(function(d: any) { return d.aviso_id === aviso.id; });
    if (destInfo.some(function(d: any) { return d.todos; })) { setEditAtribuicao("todos"); setEditSelectedUsers([]); }
    else { setEditAtribuicao("selecionar"); setEditSelectedUsers(destInfo.filter(function(d: any) { return d.usuario_id; }).map(function(d: any) { return d.usuario_id; })); }
  }
  async function salvarEdicao() {
    if (!editingAviso || !editTitulo.trim()) return;
    var updateData: any = { titulo: editTitulo, urgente: editUrgente, data_evento: editData || null };
    ...
    if (editAtribuicao === "todos") { await supabase.from("aviso_destinatarios").insert({ aviso_id: editingAviso.id, todos: true }); }
    else { var dests = editSelectedUsers.map(function(uid) { return { aviso_id: editingAviso.id, usuario_id: uid, todos: false }; }); if (dests.length > 0) await supabase.from("aviso_destinatarios").insert(dests); }
    setLista(lista.map(function(a: any) { return a.id === editingAviso.id ? Object.assign({}, a, updateData) : a; }));
    setEditingAviso(null); setSuccessMsg("Aviso atualizado!"); if (onAvisoUpdated) onAvisoUpdated();
  }
  ...
  function toggleUser(uid: string) { if (selectedUsers.includes(uid)) setSelectedUsers(selectedUsers.filter(function(id) { return id !== uid; })); else setSelectedUsers([...selectedUsers, uid]); }
  function toggleEditUser(uid: string) { if (editSelectedUsers.includes(uid)) setEditSelectedUsers(editSelectedUsers.filter(function(id) { return id !== uid; })); else setEditSelectedUsers([...editSelectedUsers, uid]); }

  function UserSelector({ users, selected, onToggle }: any) {
    return (<div style={{ marginBottom: 12, maxHeight: 120, overflowY: "auto", border: "1px solid " + COLORS.border, borderRadius: 10, padding: 8 }}>
      {users.map(function(u: any) { var sel = selected.includes(u.id); return (<div key={u.id} onClick={function() { onToggle(u.id); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 4px", cursor: "pointer", borderBottom: "1px solid " + COLORS.border }}><div style={{ width: 22, height: 22, borderRadius: 6, border: "2px solid " + (sel ? COLORS.green : COLORS.border), background: sel ? COLORS.green : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{sel && <span style={{ color: COLORS.white, fontSize: 12, fontWeight: 700 }}>&#10003;</span>}</div><div><div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text }}>{u.nome}</div><div style={{ fontSize: 11, color: COLORS.textMuted }}>{u.papel}</div></div></div>); })}
    </div>);
  }
  ...
  return (
    <div>
      ...
      {editingAviso && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: COLORS.white, borderRadius: 20, padding: 20, maxWidth: 380, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", maxHeight: "85vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              <button onClick={function() { setEditAtribuicao("todos"); setEditSelectedUsers([]); }} style={{ flex: 1, padding: "8px", borderRadius: 10, border: "none", background: editAtribuicao === "todos" ? COLORS.blue : COLORS.blueLight, color: editAtribuicao === "todos" ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Todos</button>
              <button onClick={function() { setEditAtribuicao("selecionar"); }} style={{ flex: 1, padding: "8px", borderRadius: 10, border: "none", background: editAtribuicao === "selecionar" ? COLORS.blue : COLORS.blueLight, color: editAtribuicao === "selecionar" ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Selecionar</button>
            </div>
            {editAtribuicao === "selecionar" && <UserSelector users={outrosUsuarios} selected={editSelectedUsers} onToggle={toggleEditUser} />}
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <div onClick={function() { setEditUrgente(!editUrgente); }} style={{ width: 22, height: 22, borderRadius: 6, border: "2px solid " + (editUrgente ? COLORS.pink : COLORS.border), background: editUrgente ? COLORS.pink : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>{editUrgente && <span style={{ color: COLORS.white, fontSize: 12, fontWeight: 700 }}>&#10003;</span>}</div>
            ...
            <button onClick={excluirAviso} style={{ width: "100%", padding: 10, borderRadius: 10, border: "2px solid " + COLORS.pink, background: "transparent", color: COLORS.pink, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>&#128465; Excluir aviso</button>
          </div>
        </div>
      )}
      ...
      {mostrarForm && (
        <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 16, border: "1px solid " + COLORS.border }}>
          ...
          {atribuicao === "selecionar" && <UserSelector users={outrosUsuarios} selected={selectedUsers} onToggle={toggleUser} />}
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <div onClick={function() { setUrgente(!urgente); }} style={{ width: 22, height: 22, borderRadius: 6, border: "2px solid " + (urgente ? COLORS.pink : COLORS.border), background: urgente ? COLORS.pink : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>{urgente && <span style={{ color: COLORS.white, fontSize: 12, fontWeight: 700 }}>&#10003;</span>}</div>
            <span style={{ fontSize: 13, color: COLORS.text }}>Urgente</span>
          </div>
      ...
      {lista.map(function(aviso: any) {
        return (<div key={aviso.id} style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + (aviso.urgente ? COLORS.pink + "33" : COLORS.border), borderLeft: aviso.urgente ? "4px solid " + COLORS.pink : "4px solid " + COLORS.border }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, color: COLORS.text, margin: "0 0 6px" }}>{aviso.urgente && <span style={{ color: COLORS.pink, marginRight: 6 }}>&#9679;</span>}{aviso.titulo}</h3>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 12, color: COLORS.textMuted }}>{new Date(aviso.created_at).toLocaleDateString("pt-BR")}</span>
            {aviso.data_evento && <span style={{ fontSize: 11, background: COLORS.blueLight, color: COLORS.blue, padding: "2px 8px", borderRadius: 6, fontWeight: 600 }}>&#128197; {new Date(aviso.data_evento + "T12:00:00").toLocaleDateString("pt-BR")}</span>}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={function() { setViewingAviso(aviso); }} style={{ flex: 1, padding: "6px 12px", borderRadius: 8, border: "1px solid " + COLORS.border, background: COLORS.white, fontSize: 12, fontWeight: 600, color: COLORS.blue, cursor: "pointer" }}>&#128065; Visualizar</button>
            {isEditor && <button onClick={function() { startEdit(aviso); }} style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: COLORS.blueLight, fontSize: 12, fontWeight: 600, color: COLORS.blue, cursor: "pointer" }}>&#9998; Editar</button>}
          </div>
        </div>);
      })}
    </div>
...
function ChatScreen({ turmas, user, usuarios }: any) {
  ...
  function getUserName(userId: string) {
    var u = usuarios.find(function(u2: any) { return u2.id === userId; });
    return u ? u.nome : "Desconhecido";
  }
  ...
  async function openChat(turma: any) {
    setActiveTurma(turma);
    setLoadingMsgs(true);
    var res = await supabase.from("mensagens").select("*").eq("turma_id", turma.id).order("created_at", { ascending: true });
  ...
  useEffect(function() {
    ...
    var channel = supabase.channel("chat-" + activeTurma.id).on("postgres_changes", { event: "INSERT", schema: "public", table: "mensagens", filter: "turma_id=eq." + activeTurma.id }, function(payload: any) {
      setMensagens(function(prev) {
  ...
  async function sendMessage() {
    if (!newMsg.trim() || !activeTurma || sending) return;
    setSending(true);
    await supabase.from("mensagens").insert({ turma_id: activeTurma.id, autor_id: user.id, conteudo: newMsg.trim() });
    setNewMsg("");
    setSending(false);
  }
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 160px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ width: 42, height: 42, borderRadius: 12, background: COLORS.blueLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>
            {activeTurma.nome.includes("Ber") ? "&#127868;" : activeTurma.nome.includes("Maternal") ? "&#129528;" : "&#127912;"}
          </div>
          <div>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.text, margin: 0 }}>{activeTurma.nome}</h3>
            <div style={{ fontSize: 12, color: COLORS.textMuted }}>{activeTurma.usuarios?.nome ? "Prof. " + activeTurma.usuarios.nome : ""} - {activeTurma.qtd_alunos} alunos</div>
          </div>
        </div>
        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", background: COLORS.white, borderRadius: 16, padding: 16, border: "1px solid " + COLORS.border, marginBottom: 12 }}>
          {loadingMsgs && <p style={{ fontSize: 13, color: COLORS.textMuted, textAlign: "center" }}>Carregando mensagens...</p>}
          {!loadingMsgs && mensagens.length === 0 && <p style={{ fontSize: 13, color: COLORS.textMuted, textAlign: "center", marginTop: 40 }}>Nenhuma mensagem ainda. Comece a conversa!</p>}
          {mensagens.map(function(msg: any) {
            var isMe = msg.autor_id === user.id;
            var autorNome = getUserName(msg.autor_id);
            var initials = getUserInitials(msg.autor_id);
            var time = new Date(msg.created_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
            return (
              <div key={msg.id} style={{ display: "flex", flexDirection: isMe ? "row-reverse" : "row", gap: 8, marginBottom: 12, alignItems: "flex-end" }}>
                {!isMe && <div style={{ width: 32, height: 32, borderRadius: 10, background: COLORS.blueLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: COLORS.blue, flexShrink: 0 }}>{initials}</div>}
                <div style={{ maxWidth: "75%" }}>
                  {!isMe && <div style={{ fontSize: 11, fontWeight: 600, color: COLORS.blue, marginBottom: 2, paddingLeft: 4 }}>{autorNome}</div>}
                  <div style={{ background: isMe ? COLORS.blue : COLORS.bg, color: isMe ? COLORS.white : COLORS.text, padding: "10px 14px", borderRadius: isMe ? "16px 16px 4px 16px" : "16px 16px 16px 4px", fontSize: 14, lineHeight: "1.4" }}>{msg.conteudo}</div>
                  <div style={{ fontSize: 10, color: COLORS.textMuted, marginTop: 2, textAlign: isMe ? "right" : "left", paddingLeft: 4, paddingRight: 4 }}>{time}</div>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input value={newMsg} onChange={function(e) { setNewMsg(e.target.value); }} onKeyDown={function(e) { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder="Digite uma mensagem..." style={{ flex: 1, padding: "12px 16px", borderRadius: 14, border: "2px solid " + COLORS.border, fontSize: 14, outline: "none", boxSizing: "border-box" }} />
          <button onClick={sendMessage} disabled={sending || !newMsg.trim()} style={{ width: 48, height: 48, borderRadius: 14, border: "none", background: COLORS.blue, color: COLORS.white, fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", opacity: sending || !newMsg.trim() ? 0.5 : 1, flexShrink: 0 }}>&#10148;</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {turmas.map(function(turma: any) {
        return (
          <div key={turma.id} onClick={function() { openChat(turma); }} style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border, cursor: "pointer" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ width: 32, height: 32, borderRadius: 8, background: COLORS.blueLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: COLORS.blue, flexShrink: 0 }}>&#9654;</div>
            </div>
          </div>
        );
      })}
...
function PlanosScreen({ planos: initialPlanos }: any) {
  ...
  async function updateStatus(id: string, status: string, feedback?: string) {
    var updateData: any = { status: status }; if (feedback) updateData.feedback = feedback;
    await supabase.from("planos").update(updateData).eq("id", id);
    setPlanos(planos.map(function(p: any) { return p.id === id ? Object.assign({}, p, updateData) : p; })); setFeedbackId(null); setFeedbackText("");
  }
  return (
    <div>
      <h2 style={{ fontSize: 20, fontWeight: 700, color: COLORS.text, margin: "0 0 16px" }}>Planos de Aula</h2>
      <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto" }}>
        {[{ key: "todos", label: "Todos" },{ key: "pendente", label: "Pendentes" },{ key: "aprovado", label: "Aprovados" },{ key: "revisar", label: "Revisar" }].map(function(f) { return <button key={f.key} onClick={function() { setFilter(f.key); }} style={{ padding: "8px 16px", borderRadius: 20, border: "none", background: filter === f.key ? COLORS.blue : COLORS.blueLight, color: filter === f.key ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>{f.label}</button>; })}
      </div>
      ...
      {filtered.map(function(p: any) {
        return (<div key={p.id} style={{ background: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}>
          ...
          {p.feedback && p.status === "revisar" && <div style={{ background: COLORS.yellowLight, borderRadius: 10, padding: "8px 12px", marginTop: 8 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#B8860B", marginBottom: 2 }}>Ajustes solicitados:</div><div style={{ fontSize: 13, color: COLORS.text }}>{p.feedback}</div></div>}
          {p.status === "pendente" && (<div>{feedbackId === p.id ? (<div style={{ marginTop: 12 }}><textarea value={feedbackText} onChange={function(e) { setFeedbackText(e.target.value); }} placeholder="Descreva os ajustes necessarios..." style={{ width: "100%", minHeight: 70, padding: 10, borderRadius: 10, border: "2px solid " + COLORS.yellow, fontSize: 13, outline: "none", boxSizing: "border-box", marginBottom: 8, resize: "vertical" }} /><div style={{ display: "flex", gap: 8 }}><button onClick={function() { setFeedbackId(null); setFeedbackText(""); }} style={{ flex: 1, padding: "8px", borderRadius: 10, border: "2px solid " + COLORS.border, background: "transparent", color: COLORS.text, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancelar</button><button onClick={function() { updateStatus(p.id, "revisar", feedbackText); }} style={{ flex: 1, padding: "8px", borderRadius: 10, border: "none", background: COLORS.yellow, color: COLORS.white, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Enviar</button></div></div>) : (<div style={{ display: "flex", gap: 8, marginTop: 12 }}><button onClick={function() { updateStatus(p.id, "aprovado"); }} style={{ flex: 1, padding: "8px", borderRadius: 10, border: "none", background: COLORS.green, color: COLORS.white, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>&#10003; Aprovar</button><button onClick={function() { setFeedbackId(p.id); }} style={{ flex: 1, padding: "8px", borderRadius: 10, border: "2px solid " + COLORS.yellow, background: "transparent", color: "#B8860B", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>&#8635; Pedir ajuste</button></div>)}</div>)}
        </div>);
      })}
    </div>
  );
...
function CalendarioScreen({ eventos, avisos }: any) {
  ...
  var allEventos = [...eventos, ...avisos.filter(function(a: any) { return a.data_evento; }).map(function(a: any) { return { id: "aviso-" + a.id, titulo: a.titulo, data: a.data_evento, cor: a.urgente ? COLORS.pink : COLORS.blue, fromMural: true }; })].sort(function(a: any, b: any) { return a.data > b.data ? 1 : -1; });
    ...
    <div>
      ...
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, textAlign: "center" }}>
          ...
          {Array.from({ length: daysInMonth }, function(_, i) { var day = i + 1; var dateStr = currentYear + "-" + String(currentMonth + 1).padStart(2, "0") + "-" + String(day).padStart(2, "0"); var hasEvent = eventDates.includes(dateStr); var isToday = day === today.getDate() && currentMonth === today.getMonth() && currentYear === today.getFullYear(); return <div key={day} style={{ padding: "6px 2px", borderRadius: 8, fontSize: 13, fontWeight: isToday ? 700 : 400, background: isToday ? COLORS.blue : hasEvent ? COLORS.pinkLight : "transparent", color: isToday ? COLORS.white : hasEvent ? COLORS.pink : COLORS.text }}>{day}</div>; })}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}>
        ...
        <textarea placeholder="Observacoes gerais..." style={{ width: "100%", minHeight: 80, padding: 12, borderRadius: 12, border: "2px solid " + COLORS.border, fontSize: 14, resize: "vertical", outline: "none", boxSizing: "border-box" }} />
...
function GaleriaScreen() {
  var fotos = [{ id: 1, turma: "Bercario II", desc: "Atividade sensorial", color: "#FFB8C6" },{ id: 2, turma: "Bercario II", desc: "Hora da musica", color: "#B8D4FF" },{ id: 3, turma: "Maternal I", desc: "Pintura com guache", color: "#FFE0B8" },{ id: 4, turma: "Maternal I", desc: "Parquinho", color: "#B8FFCF" },{ id: 5, turma: "Jardim I", desc: "Contacao de historia", color: "#E0B8FF" },{ id: 6, turma: "Jardim I", desc: "Atividade coletiva", color: "#FFB8B8" }];
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}><h2 style={{ fontSize: 20, fontWeight: 700, color: COLORS.text, margin: 0 }}>Galeria de Fotos</h2><button style={{ background: COLORS.pink, color: COLORS.white, border: "none", borderRadius: 12, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Upload</button></div>
      <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto" }}>{["Todas","Bercario II","Maternal I","Jardim I"].map(function(t, i) { return <button key={t} style={{ padding: "8px 16px", borderRadius: 20, border: "none", background: i === 0 ? COLORS.blue : COLORS.blueLight, color: i === 0 ? COLORS.white : COLORS.blue, fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>{t}</button>; })}</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>{fotos.map(function(foto) { return (<div key={foto.id} style={{ borderRadius: 14, overflow: "hidden", background: COLORS.white, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid " + COLORS.border }}><div style={{ height: 120, background: foto.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32 }}>&#128248;</div><div style={{ padding: 10 }}><div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text }}>{foto.desc}</div><div style={{ fontSize: 11, color: COLORS.textMuted }}>{foto.turma}</div></div></div>); })}</div>
    </div>
  );
}
...
export default function Home() {
  ...
  async function loadData(userData: any) {
    ...
    setAvisos(results[0].data || []); setPlanos(results[1].data || []); setEventos(results[2].data || []); setTurmas(results[3].data || []); setUsuarios(results[4].data || []); setDestinatarios(results[5].data || []);
    setLoading(false);
  }
  function handleLogin(userData: any) { setUser(userData); loadData(userData); }
  function handleAvisoCreated(newAviso: any) { setAvisos(function(prev) { if (prev.find(function(a) { return a.id === newAviso.id; })) return prev; return [newAviso, ...prev]; }); }
  function reloadDestinatarios() { supabase.from("aviso_destinatarios").select("*").then(function(res) { setDestinatarios(res.data || []); }); }
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      ...
      <div style={{ padding: "20px 20px 100px" }}>
        ...
        {activeTab === "mural" && <MuralScreen avisos={avisos} user={user} usuarios={usuarios} destinatarios={destinatarios} onAvisoCreated={handleAvisoCreated} onAvisoUpdated={reloadDestinatarios} onAvisoDeleted={function() { loadData(user); }} />}
