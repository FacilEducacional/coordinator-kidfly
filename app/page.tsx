...
export default function Home() {
  ...
  useEffect(function() {
    ...
    supabase.auth.getSession().then(function(res) {
      if (res.data.session) {
        ...
        supabase.from("usuarios").select("*, escolas(nome)").eq("email", email).single().then(function(profRes) {
          if (profRes.data) {
            setUser(profRes.data);
            if (profRes.data.papel !== "superadmin") {
              loadData(profRes.data);
            }
          }
          setCheckingSession(false);
        });
  ...
  if (checkingSession) return (<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg }}><div style={{ textAlign: "center" }}><div style={{ fontSize: 40, marginBottom: 12 }}>&#128203;</div><p style={{ color: C.muted, fontSize: 14 }}>Carregando...</p></div></div>);
