...
export async function POST(req: Request) {
  ...
  if (body.action === "invite") {
    ...
    var existingAuth = authUsers.data ? authUsers.data.users.find(function(u: any) { return u.email === body.email; }) : null;

    if (existingAuth) {
      authId = existingAuth.id;
      await sb.auth.admin.deleteUser(existingAuth.id);
    }

    var ir = await sb.auth.admin.inviteUserByEmail(body.email, { data: { nome: body.nome }, redirectTo: siteUrl });
    if (ir.error) {
      return NextResponse.json({ error: "Erro ao enviar convite: " + ir.error.message }, { status: 400 });
    }
    authId = ir.data.user.id;

    var existingUser = await sb.from("usuarios").select("*").eq("email", body.email).single();

    if (existingUser.data) {
      var upd = await sb.from("usuarios").update({
        nome: body.nome,
        papel: body.papel,
      ...
      if (upd.error) {
        return NextResponse.json({ error: "Erro ao atualizar usuario: " + upd.error.message }, { status: 400 });
      }

      await sb.from("permissoes_usuario").delete().eq("usuario_id", upd.data.id);

      if (body.permissoes && body.permissoes.length > 0) {
        await sb.from("permissoes_usuario").insert(body.permissoes.map(function(p: any) {
          return { usuario_id: upd.data.id, modulo: p.modulo, pode_visualizar: p.pode_visualizar, pode_editar: p.pode_editar };
        }));
      }

      return NextResponse.json({ success: true, user: upd.data });
    }

    var ur = await sb.from("usuarios").insert({
      nome: body.nome,
      email: body.email,
      papel: body.papel,
      escola_id: body.escola_id,
    ...
    if (ur.error) {
      return NextResponse.json({ error: "Erro ao criar usuario: " + ur.error.message }, { status: 400 });
    }

    if (body.permissoes && body.permissoes.length > 0) {
      await sb.from("permissoes_usuario").insert(body.permissoes.map(function(p: any) {
        return { usuario_id: ur.data.id, modulo: p.modulo, pode_visualizar: p.pode_visualizar, pode_editar: p.pode_editar };
      }));
    }
